...
class CondorSheet(object):
    ...
    def _get_wks(self, week):
        if config.GSHEET_WORKSHEET_NAME != '':
            worksheet_name = config.GSHEET_WORKSHEET_NAME
        else:
    ...
    def _update_standings(self, match, match_results):
        ...
        if standings:
            ...
            try:
                ...
                allowable_cols = [3, 6 + 2*match.week, 7 + 2*match.week, 8 + 2*match.week]
            ...
            if cell_1.col == 3:
                for cell_2 in racer_2_cells:
                    if cell_2.row == cell_1.row:
                        standings.update_cell(cell_1.row, cell_2.col - 3, score)
