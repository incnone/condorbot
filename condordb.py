...
class CondorDB(object):
    ...
    def update_match(self, match):
        try:
            ...
            params = (match.timestamp, match.flags, match.number_of_races, match.league.value,
                      self._get_racer_id(match.racer_1), self._get_racer_id(match.racer_2), match.week,)
            cursor.execute(
                "UPDATE match_data "
                "SET timestamp=%s,flags=%s,number_of_races=%s,league=%s "
    ...
    def get_racer_stats(self, racer):
        try:
            ...
            racer_1_winstats = self._modify_racer_stats(racer_id, 1, racer_stats)
            racer_2_winstats = self._modify_racer_stats(racer_id, 2, racer_stats)
            racer_wins = racer_1_winstats[1] + racer_2_winstats[1]
