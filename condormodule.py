...
class Timezone(command.CommandType):
    def __init__(self, condor_module):
        ...
        self.help_text = 'Register a time zone with your account. Usage is `.timezone <zonename>`. See <TODO> for a '\
                        'list of recognized time zones; these strings should be input exactly as-is, e.g., `.timezone US\Eastern`.'
        self._cm = condor_module
    ...
    @asyncio.coroutine
    def _do_execute(self, command):
        if not self._cm.condordb.is_registered(command.author.id):
            print('Error: register first.')
        elif len(command.args) != 1:
            print('Error in interpreting timezone: wrong command arg length.')
        else:
            tz_name = command.args[0]
            if tz_name in pytz.all_timezones:
                self._cm.condordb.register_timezone(command.author.id, tz_name)
            else:
                print('Error: timezone name not supported. See <TODO> for a list of supported names.')
